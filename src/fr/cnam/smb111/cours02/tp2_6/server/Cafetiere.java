...
public class Cafetiere extends UnicastRemoteObject implements ICafetiere {
    ...
    public void addWater(int waterQuantity) throws RemoteException {
        if(this.currentWater+waterQuantity>this.maxWater)
            throw new MaxWaterExceeded("Cette qauntite d'eau ne peut etre ajouiter car on depassera le maximum");
    ...
    public void addDose(int nbDose) throws RemoteException{
        if(this.curretnDose+nbDose>this.maxDose)
                throw new MaxDoseExceeded("Cette quantite de doses ne peut etre ajouter car on depassera le maximum");
        this.curretnDose += nbDose;
    }

    private boolean getCoffe(int waterQuantity,int doseNumeber){

        return this.currentWater>=waterQuantity && this.curretnDose >= doseNumeber;
    }
